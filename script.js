...
function approveRequest(index) {
  const requests = getStore(KEYS.REQUESTS, []);
  const resources = getStore(KEYS.RESOURCES, {}); // fixed: ensure resources is defined
  ...
  const totalAvailable = resources[resourceKey].reduce((sum, p) => sum + safeNumber(p.qty), 0);
  ...
  while (remainingReq > 0 && resources[resourceKey].length > 0) {
    const provider = resources[resourceKey][0];
    const providerQty = safeNumber(provider.qty);
    const qtyToTake = Math.min(providerQty, remainingReq);
  ...
  if (wantDelivery) {
    const logisticsList = getStore(KEYS.LOGISTICS, []);
    if (logisticsList.length) {
      const assigned = logisticsList[logisticsList.length - 1];
      const baseDateISO = request.requestedAt || new Date().toISOString();
      const scheduledISO = generateScheduledDate(baseDateISO);
      const scheduledDisplay = formatDate(scheduledISO);

      matchesToCreate.forEach((match) => {
        match.deliveryDateISO = scheduledISO;
        match.deliveryDate = scheduledDisplay;
        match.logisticsName = safeText(assigned.name, "Not Assigned");
        match.logisticsContact = safeText(assigned.contact, "N/A");
        match.status = "Scheduled";
      });

      requests[index].deliveryDate = scheduledDisplay;
      requests[index].logisticsShared = true;
    }
  }
...
function sanitizeMatch(match = {}) {
  ...
  return {
    ...
    status,
    statusClass: status.toLowerCase().includes("scheduled")
      ? "scheduled"
...
function initResourceMap() {
  ...
  resourceMap = L.map("resourceMap").setView([20.5937, 78.9629], 5);
...
function setMapLocation(map, markerName, lat, lng, latInputId, lngInputId, addressInputId) {
  ...
  if (markerName === "resourceMarker" && resourceMarker) {
    map.removeLayer(resourceMarker);
  }
  if (markerName === "requestMarker" && requestMarker) {
    map.removeLayer(requestMarker);
  }

  const marker = L.marker([lat, lng]).addTo(map);
  if (markerName === "resourceMarker") {
    resourceMarker = marker;
  } else if (markerName === "requestMarker") {
